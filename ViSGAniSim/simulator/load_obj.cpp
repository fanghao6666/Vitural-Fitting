...
Obj::~Obj()
{
	if (uni_vertices.capacity() != 0) {
		vector<glm::vec4>().swap(uni_vertices);
		vector<glm::vec2>().swap(uni_tex);
		vector<glm::vec3>().swap(uni_normals);
		vector<unsigned int>().swap(vertex_index);

		vector<glm::vec4>().swap(vertices);
		vector<glm::vec2>().swap(tex);
		vector<glm::vec3>().swap(normals);
		vector<Face>().swap(faces);

		vector<pair<string, unsigned int>>().swap(vertex_object);
		vector<pair<string, unsigned int>>().swap(face_group);
	}
...
void Obj::ChangeObj(const string file)
{
	vertices.clear();
	normals.clear();
	tex.clear();
