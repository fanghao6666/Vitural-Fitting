...
void Scene::RenderGPU_CUDA(vector<Obj> &body)
{
	if (F > MaxFrequence)
	{
		if (pose < pscene->body.size())
		{
			pscene->UpdateBodyData(body[pose++], body[0]);
			//body[pose - 1].~Obj();
		}
		else if (poser > 0)
		{
			pscene->UpdateBodyData(body[poser--], body[0]);
			body[pose+1].~Obj();
		}
		//if (MaxFrequence == 5000) {
		//	int c;
		//	cin >> c;
		//}
		//cout << pose << endl;
...
void Scene::UpdateBodyData(Obj &body, Obj &pre_body)
{
	...
	BVHAccel bvh(h_primitives);

	vector<Primitive> ().swap(h_primitives); //free the memory of primitives


	bvh_body.~Obj();//free the memory of bvh_body
	
	//free the memory of old BRTree on GPU
	cudaFree(pscene->simulation->d_leaf_nodes);
	cudaFree(pscene->simulation->d_internal_nodes);
	cudaFree(pscene->simulation->d_primitives);


	pscene->simulation->d_leaf_nodes = bvh.d_leaf_nodes;
	pscene->simulation->d_internal_nodes = bvh.d_internal_nodes;
	pscene->simulation->d_primitives = bvh.d_primitives;


	cudaError_t cudaStatus = cudaGraphicsGLRegisterBuffer(&body_vbo_resource, pre_body.vbo.array_buffer, cudaGraphicsMapFlagsWriteDiscard);   	//register vbo
	...
	body_p_normal = (glm::vec3*)((float*)body_p_vertex + 4 * pre_body.uni_vertices.size() + 2 * pre_body.uni_tex.size());   // 获取normal位置指针


	body.unified(); //归一化body否则显示有错。

	const unsigned int vertices_bytes = sizeof(glm::vec4)  * pre_body.uni_vertices.size();       //点的索引
	cudaStatus = cudaMemcpy(body_p_vertex, &body.uni_vertices[0], vertices_bytes, cudaMemcpyHostToDevice);

	const unsigned int normal_bytes = sizeof(glm::vec3)  * pre_body.uni_normals.size();       //点的索引
	cudaStatus = cudaMemcpy(body_p_normal, &body.uni_normals[0], normal_bytes, cudaMemcpyHostToDevice);
